...
const currentCompanyId = () => {
    if (state.subscribeToggle === "whole")
        return companyList[state.bigCtg][state.smallCtg].id;
...
const switchNextCtg = () => {
    if (state.subscribeToggle === "whole") {
        ...
        if (state.smallCtg >= companyList[state.bigCtg].length) {
            state.bigCtg++;
            if (state.bigCtg >= companyList.length)
                state.bigCtg = 0;
            state.smallCtg = 0;
        }
    }
...
const switchPrevCtg = () => {
    if (state.subscribeToggle === "whole") {
        ...
        if (state.smallCtg < 0) {
            state.bigCtg--;
            if (state.bigCtg < 0)
                state.bigCtg = companyList.length - 1;
        ...
        state.bigCtg--;
        if (state.bigCtg < 0)
            state.bigCtg = getSubscribeList().length - 1;
    }
};
...
export default function CategoriesAndNewsSection(_news, _newsCom) {
    ...
    document.querySelector(".subscribe-toggle").addEventListener("click", (e) => {
        ...
        if (targetClassName !== "whole" && targetClassName !== "my")
            return;
